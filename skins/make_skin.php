...
function ois_make_skin($skin, $design) {
	...
	if (isset($skin['disable_mobile'])) {
		$responsiveness = $skin['disable_mobile'];
	} else {
		$responsiveness = '';
	}
	$data = array (
...
function ois_wrapper($ois, $data) {
	...
	// Responsiveness
	$resp_class = ' ';
	if (!empty($data['responsiveness'])) { 
		// see if user has set responsiveness option.
		// Cases: show_large, show_small, show_all
		if ($data['responsiveness'] == 'show_large') { 
			$resp_class .= 'visible-md visible-lg';
		} else if ($data['responsiveness'] == 'show_small') { 
			$resp_class .= 'visible-sm visible-xs';
		}
	} else {
